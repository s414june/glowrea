...
function routerToServer() {
  if (!server.value)
    return
  navigateTo({ path: `/${server.value}` })
}
...
<template>
  <form text-center justify-center items-center max-w-150 py6 flex="~ col gap-3" @submit.prevent="routerToServer">
    ...
    <div :class="error ? 'animate animate-shake-x animate-delay-100' : null">
      <div
        dir="ltr" flex bg-gray:10 px4 py2 mxa rounded border="~ base" items-center font-mono focus:outline-none
        focus:ring="2 primary inset" relative :class="displayError ? 'border-red-600 dark:border-red-400' : null"
      >
        ...
        <input
          ...
          max-w-50 spellcheck="false" autocorrect="off" autocomplete="off" @input="handleInput" @keydown.down="move(1)"
          @keydown.up="move(-1)" @keydown.enter="onEnter" @keydown.esc.prevent="escapeAutocomplete"
        ...
        <div
          ...
          border="~ base" z-10 shadow of-auto overflow-y-auto class="max-h-[8rem]"
        >
          <button
            v-for="(name, idx) in filteredServers" :id="toSelector(name)" :key="name" :value="name" px-2 py1
            font-mono w-full text-left :class="autocompleteIndex === idx ? 'text-primary font-bold' : null"
            @click="select(idx)"
          >
            {{ name }}
          </button>
    ...
    <div text-secondary text-sm flex>
      ...
      <span>
        <NuxtLink
          href="https://joinmastodon.org/servers" target="_blank" external class="text-primary"
          hover="underline"
        >{{ $t('user.tip_search_server') }}</NuxtLink>
      </span>
    </div>
