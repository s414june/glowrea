...
onMounted(() => {
  watch(() => statusBodyComponent.value, async () => {
    if (statusBodyComponent.value) {
      const originHeight = statusBodyComponent.value.scrollHeight
      statusBodyComponent.value.dataset.clip = 'true'

      const newHeight = statusBodyComponent.value?.scrollHeight
      if (originHeight - newHeight < 1) {
        statusBodyComponent.value.dataset.clip = 'false'
      }
      else {
        hasMore.value = true
      }
...
<template>
  <ClientOnly>
    ...
    >
      <span
        v-if="status.content" class="content-rich line-compact" dir="auto"
